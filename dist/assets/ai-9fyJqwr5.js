import{s as e}from"./graphql-REqfkFwC.js";import{Bn as t,Jn as n,Sn as r,Yn as i,un as a,vn as o}from"./index-CbvN_Jmh.js";import{toMp3 as s,transcription as c}from"./audio-M0-QB1fe.js";var l=e`
...
`;function f(e){let u=r(),d=o(),{$gettext:f}=i;if(!u.can(`audio:transcribe`)){d.add(f(`Permission denied`),`error`);return}return s(n(e,!0)).then(e=>a.mutate({mutation:l,variables:{file:new File([e],`audio.mp3`,{type:`audio/mpeg`})},context:{hasUpload:!0}})).then(e=>{if(e.errors)throw e;return c(t(e.data?.transcribe||`[]`,[]))}).catch(e=>{d.add(f(`Error transcribing file`)+`:
`+e,`error`),console.error(`useAi::transcribe(): Error transcribing from media URL`,e)})}function p(e,t,n=null,s=null){let c=r(),l=o(),{$gettext:d}=i;if(!c.can(`text:translate`)){l.add(d(`Permission denied`),`error`);return}return Array.isArray(e)||(e=[e].filter(e=>!!e)),e.length?t?a.mutate({mutation:u,variables:{texts:e,to:t.toUpperCase(),from:n?.toUpperCase(),context:s}}).then(e=>{if(e.errors)throw e;return e.data?.translate||[]}).catch(e=>{l.add(d(`Error translating texts`)+`:
